...
def extract_spectrum_LRIS(filenamei,filenamew,mupoly,sigpoly,dispaxis="x",apmin=-4.,apmax=4.,
                     weight='gauss', sky=None, gain=1.0, rdnoise=0.0,
                     do_plot=True, do_subplot=False, outfile=None):
   """
   Third step in reduction process.
   ...
   the trace_spectrum function.  In particular, it takes the two
   polynomials generated by trace_spectrum as the inputs mupoly and sigpoly.
   This code takes as input the _bgsub and _var images produced by Matt's LRIS reduction code
   """
   ...
   fitx = np.arange(datai.shape[specaxis]).astype(np.float32)
   mu = 0.0 * fitx
   for i in range(mupoly.size):
      mu += mupoly[i] * fitx**(mupoly.size - 1 - i)
   sig = 0.0 * fitx
   ...
   for i in pix:
      ...
      if (sky == None):
         ...
         skyval = sky[i]
      amp[i],var[i] = extract_wtsum_col_LRIS(tmpdata,tmpdataw,mu[i],apmin,apmax,sig=sig[i],
                                        gain=gain,rdnoise=rdnoise,sky=skyval,
                                        weight=weight)
   print "   Done"
   ...
   # Save the extracted spectrum
   if outfile is not None:
      save_spectrum(outfile,wavelength,amp,var)
...
def extract_wtsum_col_LRIS(spatialdat,spatialdatw,mu,apmin,apmax,weight='gauss',sig=1.0,
                      gain=1.0,rdnoise=0.0,sky=None):
   """
   Extracts the spectrum from one row/column in the wavelength direction
   via a weighted sum.  The choices for the weighting are:
   ...
   Inputs:
     spatialdat - a one-dimensional array, corresponding to a cut in
                  the spatial direction from the 2-d spectrum
     spatialdat - same, but from the variance image
     mu         - the fixed centroid of the trace
     apmin      - the lower bound, with respect to mu, of the aperture to be
                  extracted
     apmax      - the upper bound, with respect to mu, of the aperture to be
...
def response_LRIS(obs,model,outfile,order=12,fitrange=None,filtwidth=9):
   """
   Calculates the response function for an LRIS spectral setup using observations
   of a standard star.
   The steps are:
     (1) Divide the model spectrum by the observed spectrum
     (2) Fit a polynomial to the result
   ...
   Inputs:
      obs:    Input file containing observed spectrum of the hot star
                  This file should have 3 columns (wavelength, flux, variance)
      mod:       model spectrum of the observed star
      outfile:   Output file that will contain the response function
      order:     Order of polynomial fit (default = 6)
      fitrange:  A list of 2-element lists, where each of the smaller lists
                  contains the starting and ending values for a range of
   ...
   # Read the input spectrum
   wave,fluxobs,var = read_spectrum(obs)
   rms = np.sqrt(var)

   # Read the model spectrum and interpolate it on the grid of the observed spectrum
   wave_,fluxobs_,ignore_ = read_spectrum(model)
   f = interpolate.interp1d(wave_, fluxobs_)
   ...
   # Convert polynomial into a smooth response function
   p = np.poly1d(fpoly)
   #resp = p(wave) # USE THIS ONE IF YOU WANT TO USE THE POLYNOMIAL FIT
   resp = respgood
...
def redbluenorm_LRIS(blue__,red__,blue_response__,red_response__,out,blue_ord,red_ord):
   """
   Combines the blue and red sides, using a response function. The response function is interpolated on the wavelength grid of the input. Run this after running spec_simple.find_and_trace, spec_simple.extract_spectrum_LRIS and spec_simple.response_LRIS.
   blue__,red__: input blue/red spectrum; text file containing wavelength, flux and variance
   blue_response__,red_response__: input blue/red response; text file containing wavelength and response columns
   out: name of output file
   blue_ord,red_ord: polynomial orders used to produce the response functions
   """

   # Read the data and interpolate the response; inside the wavelenght range covered by the original response perform interpolation, and outside extrapolate with a polynomial fit
                       
   blue=np.loadtxt(blue__,usecols=[0,1,2],unpack=True)
   resp_blue_=np.loadtxt(blue_response__,usecols=[0,1],unpack=True)
   red=np.loadtxt(red__,usecols=[0,1,2],unpack=True)
   resp_red_=np.loadtxt(red_response__,usecols=[0,1],unpack=True)
   f = np.polyfit(resp_blue_[0], resp_blue_[1], blue_ord)
   f_interp = interpolate.interp1d(resp_blue_[0], resp_blue_[1])
   p = np.poly1d(f)
   resp_blue = np.zeros(len(blue[0]))
   resp_blue[blue[0]<np.min(resp_blue_[0])] = p(blue[0][blue[0]<np.min(resp_blue_[0])])
   resp_blue[blue[0]>np.max(resp_blue_[0])] = p(blue[0][blue[0]>np.max(resp_blue_[0])])
   resp_blue[(blue[0]<=np.max(resp_blue_[0])) & (blue[0]>=np.min(resp_blue_[0]))] = f_interp(blue[0][(blue[0]<=np.max(resp_blue_[0])) & (blue[0]>=np.min(resp_blue_[0]))])
   f = np.polyfit(resp_red_[0], resp_red_[1], red_ord)
   f_interp = interpolate.interp1d(resp_red_[0], resp_red_[1])
   p = np.poly1d(f)
   #resp_red = p(red[0])
   resp_red = np.zeros(len(red[0]))
   resp_red[red[0]<np.min(resp_red_[0])] = p(red[0][red[0]<np.min(resp_red_[0])])
   resp_red[red[0]>np.max(resp_red_[0])] = p(red[0][red[0]>np.max(resp_red_[0])])
   resp_red[(red[0]<=np.max(resp_red_[0])) & (red[0]>=np.min(resp_red_[0]))] = f_interp(red[0][(red[0]<=np.max(resp_red_[0])) & (red[0]>=np.min(resp_red_[0]))])
                   
   # Normalize and combine the blue and red sides
                       
   final_red=red[1]*resp_red
   final_blue=blue[1]*resp_blue
   final_red_var=red[2]*(resp_red**2)
   final_blue_var=blue[2]*(resp_blue**2)
   plt.plot(red[0],final_red)
   plt.plot(red[0],np.sqrt(red[2]))
   plt.plot(blue[0],final_blue)
   plt.plot(blue[0],np.sqrt(blue[2]))
   plt.show()
   lim=float(raw_input('Enter transition wavelength between blue and red:'))
   blue_=blue[0]
   red_=red[0]
   comb=np.zeros([3,np.where(blue_==np.max(blue_[blue_<lim]))[0][0]+len(red_)-np.where(red_==np.min(red_[red_>lim]))[0][0]])
   comb[0][0:np.where(blue_==np.max(blue_[blue_<lim]))[0][0]]=blue[0][0:np.where(blue_==np.max(blue_[blue_<lim]))[0][0]]
   comb[1][0:np.where(blue_==np.max(blue_[blue_<lim]))[0][0]]=final_blue[0:np.where(blue_==np.max(blue_[blue_<lim]))[0][0]]
   comb[2][0:np.where(blue_==np.max(blue_[blue_<lim]))[0][0]]=final_blue_var[0:np.where(blue_==np.max(blue_[blue_<lim]))[0][0]]
   comb[0][np.where(blue_==np.max(blue_[blue_<lim]))[0][0]:len(comb[0])]=red[0][np.where(red_==np.min(red_[red_>lim]))[0][0]:len(red_)]
   comb[1][np.where(blue_==np.max(blue_[blue_<lim]))[0][0]:len(comb[0])]=final_red[np.where(red_==np.min(red_[red_>lim]))[0][0]:len(red_)]
   comb[2][np.where(blue_==np.max(blue_[blue_<lim]))[0][0]:len(comb[0])]=final_red_var[np.where(red_==np.min(red_[red_>lim]))[0][0]:len(red_)]
   np.savetxt(out,comb.T,fmt='%.2f %.2f %.2f')
   #plt.clf()
   #plt.plot(comb[0],comb[1])
   #plt.plot(comb[0],np.sqrt(comb[2]))
   #plt.show()
   #plt.plot(red[0],resp_red)
   #plt.show()
...
def complete_LRIS(blue,red,blue_var,red_var,resp_blue,resp_red,out,blue_ord,red_ord):
   """
   Given the input 2d spectra in blue and red and the response function, extracts the trace, applies the response function and combines the blue and red sides.
   out: required format is the galaxy name, without prefix or extension
   """
                       
   datai=load_2d_spectrum(red)
   dataw=load_2d_spectrum(red_var)
   datai[np.isnan(datai)]=0
   dataw[np.isnan(dataw)]=0
   mupoly,sigpoly=find_and_trace(datai, dispaxis="x", apmin=-4., apmax=4., stepsize=25,muorder=3, sigorder=4, fitrange=None, do_plot=True,do_subplot=False)
   amp,var=extract_spectrum_LRIS(red,red_var,mupoly,sigpoly,dispaxis="x",apmin=-4.,apmax=4.,weight='gauss', sky=None, gain=1.0, rdnoise=0.0, do_plot=True, do_subplot=False, outfile="red_%s_1d.dat" %out)
   datai=load_2d_spectrum(blue)
   dataw=load_2d_spectrum(blue_var)
   datai[np.isnan(datai)]=0
   dataw[np.isnan(dataw)]=0
   mupoly,sigpoly=find_and_trace(datai, dispaxis="x", apmin=-4., apmax=4., stepsize=25,muorder=3, sigorder=4, fitrange=None, do_plot=True,do_subplot=False)
   amp,var=extract_spectrum_LRIS(blue,blue_var,mupoly,sigpoly,dispaxis="x",apmin=-4.,apmax=4.,weight='gauss', sky=None, gain=1.0, rdnoise=0.0, do_plot=True, do_subplot=False, outfile="blue_%s_1d.dat" %out)
   redbluenorm_LRIS("blue_%s_1d.dat" %out,"red_%s_1d.dat" %out,resp_blue,resp_red,"%s_comb.dat" %out,6,30)
   spec=np.loadtxt("%s_comb.dat" %out,usecols=[0,1,2],unpack=True)
   plt.clf()
   plt.plot(spec[0],spec[1])
   plt.plot(spec[0],spec[2])
   plt.ion() # keeps the plot open so I can overplot emission lines using mark_spec_emission
   plt.show()
   smooth_boxcar("%s_comb.dat" %out, filtwidth=10, outfile=None, varwt=True)
...
def make_2gauss_plus_bkgd(x,mu,sigma,amp,bkgd,mu1,sigma1,amp1):
   """
   Creates 2 gaussians plus background model given input x and parameters.
   The parameter values are:
    mu
    sigma
    amplitude
   ...
   """ Calculate y_mod using current parameter values """
   ymod = bkgd + amp * np.exp(-0.5 * ((x - mu)/sigma)**2) + amp1 * np.exp(-0.5 * ((x - mu1)/sigma1)**2)
   return ymod
                        
def make_2gauss_plus_bkgd_fixparam(x,mu,sigma,amp,bkgd,amp1,offset):
   """
   Creates 2 gaussians plus background model given input x and parameters.
   The parameter values are:
    mu
   ...
   """ Calculate y_mod using current parameter values """
   ymod = bkgd + amp * np.exp(-0.5 * ((x - mu)/sigma)**2) + amp1 * np.exp(-0.5 * ((x - mu-offset)/sigma)**2) # force same sigma for both Gaussians, and relative offset
   return ymod
...
def fit_2gauss_plus_bkgd(p,x,y):
   """
   Compares the data to the model.  The model is 2 gaussians plus a
    constant background.
   The parameter values are:
    p[0] = background level
    p[1] = mu
   ...
   ymod = make_2gauss_plus_bkgd(x,mu,sig,amp,bkgd,mu1,sig1,amp1)
   #diff = y - ymod
   if (sig1 < 0): diff = np.inf                     #if (sig1 !=  sig) or (sig1 < 0): diff = np.inf
   else: diff = y - ymod
   return diff

def fit_2gauss_plus_bkgd_fixparam(p,x,y,offset):
   """
   Compares the data to the model.  The model is 2 gaussians plus a
    constant background.
   The parameter values are:
   ...
   ymod = make_2gauss_plus_bkgd_fixparam(x,mu,sig,amp,bkgd,amp1,offset)
   if (amp1 < 0): diff = np.inf
   else: diff = y - ymod
   return diff
...
def find_2blendedpeaks(data,dispaxis="x",offset=None,mu0=None,sig0=None,fixmu=False,fixsig=False,
   showplot=True,do_subplot=False,verbose=True,apmin=-4.,apmax=4.):
   """
    Compresses a 2d spectrum along the dispersion axis so that
     the traces of the spectrum can be automatically located by fitting
     a gaussian + background to the spatial direction.  The function
     returns the parameters of the best-fit gaussians.
    The default dispersion axis is along the x direction.  To change this
     set the optional parameter dispaxis to "y"
    offset: guess of the position of the second trace relative to the first, in pixels
   """

   try:
      offset_ = float(offset)
   except:
      print "You need to set the relative offset for the second trace in pixels!"
   ...
   if(verbose):
      ...
      print "Initial guesses for Gaussian plus background fit"
      print "------------------------------------------------"
      print " mu         = %7.2f%7.2f%s"   % (mu0,mu1,fixmunote)
      print " sigma      =   %5.2f%5.2f%s" % (sig0,sig1,fixsignote)
      print " amplitude of main trace  = %f"        % amp0
      print " background = %f"        % bkgd0
      print "Parameters marked with a ** are held fixed during the fit"
      print ""
   ...
   #p_out,ier = optimize.leastsq(fit_gpb_fixsig,p,(x,cdat,sig0),maxfev=mf)
      ...
      p = [bkgd0,mu0,sig0,amp0,mu1,sig1,amp1]
      p_out,ier = optimize.leastsq(fit_2gauss_plus_bkgd,p,(x,cdat),maxfev=mf)
   ...
   if(verbose):
      print "Fitted values for Gaussian plus background fit"
      print "----------------------------------------------"
      print " mu         = %7.2f%7.2f%s"   % (p_out[1],p_out[4],fixmunote)
      print " sigma      =   %5.2f%5.2f%s" % (p_out[2],p_out[5],fixsignote)
      print " amplitude  = %f %f"        % (p_out[3],p_out[6])
      print " background = %f"        % p_out[0]
      print "Parameters marked with a ** are held fixed during the fit"
      print ""
   ...
   if(showplot):
      ...
      plt.plot(x,cdat,linestyle='steps')
      xmod = np.arange(1,cdat.shape[0]+1,0.1)
      ymod = make_2gauss_plus_bkgd(xmod,p_out[1],p_out[2],p_out[3],p_out[0],p_out[4],p_out[5],p_out[6])
      plt.plot(xmod,ymod)
      plt.axvline(p_out[1]+apmin,color='k')
      plt.axvline(p_out[1]+apmax,color='k')
      plt.xlabel('Pixel number in the spatial direction')
...
def find_2blendedpeaks_fixparam(data,dispaxis="x",offset=None,mu0=None,sig0=None,fixmu=False,fixsig=False,
   showplot=True,do_subplot=False,verbose=True,apmin=-4.,apmax=4.):
   """
    Compresses a 2d spectrum along the dispersion axis so that
     the traces of the spectrum can be automatically located by fitting
   ...
   if(verbose):
      ...
      print "Initial guesses for Gaussian plus background fit"
      print "------------------------------------------------"
      print " mu         = %7.2f%7.2f%s"   % (mu0,mu0+offset,fixmunote)
      print " sigma      =   %5.2f%5.2f%s" % (sig0,sig0,fixsignote)
      print " amplitude of main trace  = %f"        % amp0
      print " background = %f"        % bkgd0
      print "Parameters marked with a ** are held fixed during the fit"
      print ""
   ...
   #p_out,ier = optimize.leastsq(fit_gpb_fixsig,p,(x,cdat,sig0),maxfev=mf)
      ...
      p = [bkgd0,mu0,sig0,amp0,amp1]
                        #p = [0,13,1,1700,250]
      p_out,ier = optimize.leastsq(fit_2gauss_plus_bkgd_fixparam,p,(x,cdat,offset_),maxfev=mf)

   # Give results
   if(verbose):
      print "Fitted values for Gaussian plus background fit"
      print "----------------------------------------------"
      print " mu         = %7.2f%7.2f%s"   % (p_out[1],p_out[1] + offset_,fixmunote)
      print " sigma      =   %5.2f%5.2f%s" % (p_out[2],p_out[2],fixsignote)
      print " amplitude  = %f %f"        % (p_out[3],p_out[4])
      print " background = %f"        % p_out[0]
      print "Parameters marked with a ** are held fixed during the fit"
      print ""
   ...
   if(showplot):
      ...
      plt.plot(x,cdat,linestyle='steps')
      xmod = np.arange(1,cdat.shape[0]+1,0.1)
      ymod = make_2gauss_plus_bkgd_fixparam(xmod,p_out[1],p_out[2],p_out[3],p_out[0],p_out[4],offset_)
      plt.plot(xmod,ymod)
      plt.axvline(p_out[1]+apmin,color='k')
      plt.axvline(p_out[1]+apmax,color='k')
      plt.xlabel('Pixel number in the spatial direction')
...
#-----------------------------------------------------------------------
def find_2blendedtraces(data,dispaxis="x",offset=None,apmin=-4.,apmax=4.,do_plot=True,
               do_subplot=False):
   """
   First step in the reduction process.
   Runs find_peak on the full 2d spectrum in order to set the initial
    guess for trace_spectrum.
   """
   p = find_2blendedpeaks(data,dispaxis=dispaxis,offset=offset,apmin=apmin,apmax=apmax,
                 showplot=do_plot,do_subplot=do_subplot)
   mu0  = p[1]
   sig0 = p[2]
   mu1  = p[4]
...
def find_2blendedtraces_fixparam(data,dispaxis="x",offset=None,apmin=-4.,apmax=4.,do_plot=True,
               do_subplot=False):
   """
   First step in the reduction process.
   Runs find_peak on the full 2d spectrum in order to set the initial
    guess for trace_spectrum.
   """
   p = find_2blendedpeaks_fixparam(data,dispaxis=dispaxis,offset=offset,apmin=apmin,apmax=apmax,
                 showplot=do_plot,do_subplot=do_subplot)
   mu0  = p[1]
   sig0 = p[2]
   mu1  = p[1]+offset
...
def blendedtraces_spectrum(data,mu0,sig0,mu1,sig1,dispaxis="x",offset=None,stepsize=25,muorder=3,
   sigorder=4,fitrange=None,do_plot=True,do_subplot=False):
   """
   Second step in the reduction process.
   Fits a gaussian plus background to portions of the spectrum separated
   ...
   for i in nsteps:
      if(specaxis == 0):
         ...
         tmpdata = data[:,xstep[i]:xstep[i]+stepsize]
      ptmp = find_2blendedpeaks(tmpdata,dispaxis=dispaxis,offset=offset_,showplot=False,verbose=False)
      mu[i]    = ptmp[1]
      sigma[i] = ptmp[2]
      mu1new[i]    = ptmp[4]
      sigma1new[i] = ptmp[5]
   ...
   print "Fitting a polynomial of order %d to the location of the trace" \
       % muorder
   mupoly = fit_poly_to_trace(xstep,mu,muorder,mu0,xlength,fitrange,markformat='go',title='Location of Peak',
                                                           ylabel='Centroid of trace',
                                                           do_plot=do_plot)
   plt.show()
   mupoly1 = fit_poly_to_trace(xstep,mu1new,muorder,mu1,xlength,fitrange,markformat='go',title='Location of Peak',
                                                            ylabel='Centroid of trace',
                                                            do_plot=do_plot)
   plt.show()
   # Fit a polynomial to the width of the trace
   ...
   sigpoly = fit_poly_to_trace(xstep,sigma,sigorder,sig0,xlength,fitrange,
                               ...
                               ylabel='Width of trace (Gaussian sigma)',
                               do_plot=do_plot)
   sigpoly1 = fit_poly_to_trace(xstep,sigma1new,sigorder,sig1,xlength,fitrange,
                               markformat='go',title='Width of Peak',
                               ylabel='Width of trace (Gaussian sigma)',
                               do_plot=do_plot)
   plt.show()
   # Return the fitted parameters
   return mupoly,sigpoly,mupoly1,sigpoly1
                        
                        
def blendedtraces_spectrum_fixparam(data,mu0,sig0,mu1,sig1,dispaxis="x",offset=None,stepsize=25,muorder=3,
   sigorder=4,fitrange=None,do_plot=True,do_subplot=False):
   """
   Second step in the reduction process.
   Fits a gaussian plus background to portions of the spectrum separated
   ...
   for i in nsteps:
      if(specaxis == 0):
         ...
         tmpdata = data[:,xstep[i]:xstep[i]+stepsize]
      ptmp = find_2blendedpeaks_fixparam(tmpdata,dispaxis=dispaxis,offset=offset_,showplot=False,verbose=False)
      mu[i]    = ptmp[1]
      sigma[i] = ptmp[2]
      mu1new[i]    = ptmp[1]+offset
      sigma1new[i] = ptmp[2]
   print "   Done"
...
def find_and_traceblended(data, dispaxis="x", offset=None, apmin=-4., apmax=4., stepsize=25,
                   muorder=3, sigorder=4, fitrange=None, do_plot=True,
                   do_subplot=False):
   ...
   try:
      ...
      print "You need to set the relative offset for the second trace in pixels!"
   mu0,sig0,mu1,sig1 = find_2blendedtraces(data,dispaxis,offset_,apmin,apmax,do_plot,do_subplot)

   pos,width,pos1,width1 = blendedtraces_spectrum(data,mu0,sig0,mu1,sig1,dispaxis,offset_,stepsize,muorder,
                              sigorder,fitrange,do_plot,do_subplot)

   return pos,width,pos1,width1
                        
def find_and_traceblended_fixparam(data, dispaxis="x", offset=None, apmin=-4., apmax=4., stepsize=25,
                   muorder=3, sigorder=4, fitrange=None, do_plot=True,
                   do_subplot=False):
   ...
   try:
      ...
      print "You need to set the relative offset for the second trace in pixels!"
   mu0,sig0,mu1,sig1 = find_2blendedtraces_fixparam(data,dispaxis,offset_,apmin,apmax,do_plot,do_subplot)

   pos,width,pos1,width1 = blendedtraces_spectrum_fixparam(data,mu0,sig0,mu1,sig1,dispaxis,offset_,stepsize,muorder,
                              sigorder,fitrange,do_plot,do_subplot)
...
def extractblendedtraces_spectrum(filenamei,filenamew,mupoly,sigpoly,mupoly1,sigpoly1,dispaxis="x",apmin=-4.,apmax=4.,
                                                  weight='gauss', sky=None, gain=1.0, rdnoise=0.0,
                                                  do_plot=True, do_subplot=False, outfile=None, prefix=None):
   """
    Third step in reduction process.
    This function extracts a 1D spectrum from the input 2D spectrum (data)
    It uses the information about the trace that has been generated by
    the blendedtraces_spectrum function.  In particular, it takes the four
    polynomials generated by trace_spectrum as the inputs mupoly and sigpoly, mupoly1 and sigpoly1.
   This code takes as input the _bgsub and _var images produced by Matt's LRIS reduction code
   """
   ...
   for i in range(mupoly.size):
      mu += mupoly[i] * fitx**(mupoly.size - 1 - i)
      mu1 += mupoly1[i] * fitx**(mupoly1.size - 1 - i)
   sig = 0.0 * fitx
   sig1 = 0.0 * fitx
   for i in range(sigpoly.size):
      sig += sigpoly[i] * fitx**(sigpoly.size - 1 - i)
      sig1 += sigpoly1[i] * fitx**(sigpoly1.size - 1 - i)
   ...
   for i in pix:
      ...
      if (sky == None):
         ...
         skyval = sky[i]
      amp[i],var[i],amp1[i],var1[i] = extractblendedtraces_wtsum_col_LRIS(tmpdata,tmpdataw,mu[i],mu1[i],apmin,apmax,sig=sig[i],sig1=sig1[i],
                                                                                            gain=gain,rdnoise=rdnoise,sky=skyval,weight=weight)
   print "   Done"
   ...
   if do_plot:
      ...
      plot_spectrum_array(pix,amp,title='Extracted spectrum (pixels)',
                          xlabel='Pixel number in the dispersion direction')
      plot_spectrum_array(pix,amp1,title='Extracted spectrum (pixels)',
                                            xlabel='Pixel number in the dispersion direction')
      plt.show()

   # Save the extracted spectrum
   if outfile is not None:
      save_spectrum("%s_%s_trace1.dat" %(prefix,outfile),wavelength,amp,var)
      save_spectrum("%s_%s_trace2.dat" %(prefix,outfile),wavelength,amp1,var1)

   # Return the extracted spectrum
   return amp,var,amp1,var1

#-----------------------------------------------------------------------
                        
#-----------------------------------------------------------------------
                        
def extractblendedtraces_wtsum_col_LRIS(spatialdat,spatialdatw,mu,mu1,apmin,apmax,weight='gauss',sig=1.0,sig1=1.0,
                      gain=1.0,rdnoise=0.0,sky=None):
   """
   Extracts the spectrum from one row/column in the wavelength direction
   via a weighted sum.  The choices for the weighting are:
   ...
   if(weight == 'uniform'):
      ...
      gweight = make_gauss_plus_bkgd(y,mu,sig,1.0,0.0)
      gweight1 = make_gauss_plus_bkgd(y,mu1,sig1,1.0,0.0)

   """ Do the weighted sum """
   wtsum = ((spatialdat - bkgd)*gweight)[apmask].sum() / gweight[apmask].sum()
   wtsum1 = ((spatialdat - bkgd)*gweight1)[apmask].sum() / gweight1[apmask].sum()

   """ Calculate the variance """
   varspec = spatialdatw
   var = (varspec * gweight)[apmask].sum() / gweight[apmask].sum()
   var1 = (varspec * gweight1)[apmask].sum() / gweight1[apmask].sum()

   return wtsum,var,wtsum1,var1

#-----------------------------------------------------------------------

#-----------------------------------------------------------------------

def completeblendedtraces_LRIS(blue,red,blue_var,red_var,resp_blue,resp_red,offset,out,blue_ord,red_ord):
   """
   Given the input 2d spectra in blue and red and the response function, extracts the trace, applies the response function and combines the blue and red sides.
   out: required format is the galaxy name, without prefix or extension
   """
   ...
   datai[np.isnan(datai)]=0
   dataw[np.isnan(dataw)]=0
   mupoly,sigpoly,mupoly1,sigpoly1=find_and_traceblended(datai, dispaxis="x", offset=offset, apmin=-4., apmax=4., stepsize=25,muorder=3, sigorder=4, fitrange=None, do_plot=True,do_subplot=False)
   extractblendedtraces_spectrum(red,red_var,mupoly,sigpoly,mupoly1,sigpoly1,dispaxis="x",apmin=-4.,apmax=4.,weight='gauss', sky=None, gain=1.0, rdnoise=0.0, do_plot=True, do_subplot=False, outfile=out, prefix="red")
   datai=load_2d_spectrum(blue)
   dataw=load_2d_spectrum(blue_var)
   datai[np.isnan(datai)]=0
   dataw[np.isnan(dataw)]=0
   mupoly,sigpoly,mupoly1,sigpoly1=find_and_traceblended(datai, dispaxis="x", offset=offset, apmin=-4., apmax=4., stepsize=25,muorder=3, sigorder=4, fitrange=None, do_plot=True,do_subplot=False)
   extractblendedtraces_spectrum(blue,blue_var,mupoly,sigpoly,mupoly1,sigpoly1,dispaxis="x",apmin=-4.,apmax=4.,weight='gauss', sky=None, gain=1.0, rdnoise=0.0, do_plot=True, do_subplot=False, outfile=out, prefix="blue")
   redbluenorm_LRIS("blue_%s_trace1.dat" %out,"red_%s_trace1.dat" %out,resp_blue,resp_red,"%s_trace1_comb.dat" %out,6,30)
   redbluenorm_LRIS("blue_%s_trace2.dat" %out,"red_%s_trace2.dat" %out,resp_blue,resp_red,"%s_trace2_comb.dat" %out,6,30)
   spec1=np.loadtxt("%s_trace1_comb.dat" %out,usecols=[0,1],unpack=True)
   spec2=np.loadtxt("%s_trace2_comb.dat" %out,usecols=[0,1],unpack=True)
   plt.clf()
   plt.plot(spec1[0],spec1[1])
   plt.plot(spec2[0],spec2[1])
   plt.ion() # keeps the plot open so I can overplot emission lines using mark_spec_emission
   plt.show()
   smooth_boxcar("%s_trace1_comb.dat" %out, filtwidth=10, outfile=None, varwt=True)
   smooth_boxcar("%s_trace2_comb.dat" %out, filtwidth=10, outfile=None, varwt=True)
                        
def completeblendedtraces_LRIS_fixparam(blue,red,blue_var,red_var,resp_blue,resp_red,offset,out,blue_ord,red_ord):
   """
   Given the input 2d spectra in blue and red and the response function, extracts the trace, applies the response function and combines the blue and red sides.
   out: required format is the galaxy name, without prefix or extension
   """
   ...
   datai[np.isnan(datai)]=0
   dataw[np.isnan(dataw)]=0
   mupoly,sigpoly,mupoly1,sigpoly1=find_and_traceblended_fixparam(datai, dispaxis="x", offset=offset, apmin=-4., apmax=4., stepsize=25,muorder=3, sigorder=4, fitrange=None, do_plot=True,do_subplot=False)
   extractblendedtraces_spectrum(red,red_var,mupoly,sigpoly,mupoly1,sigpoly1,dispaxis="x",apmin=-4.,apmax=4.,weight='gauss', sky=None, gain=1.0, rdnoise=0.0, do_plot=True, do_subplot=False, outfile=out, prefix="red")
   datai=load_2d_spectrum(blue)
   dataw=load_2d_spectrum(blue_var)
   datai[np.isnan(datai)]=0
